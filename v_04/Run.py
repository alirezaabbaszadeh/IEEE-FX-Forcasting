# Run.py for Version 4 (no csv/4/)
# Main script to configure, execute, and manage the time series forecasting pipeline
# for Model Version 4. This script centralizes hyperparameter settings, drawing
# structural inspiration from Version 8's Run.py for consistency and best practices.
# Key features for V4: configurable residual blocks with internal MultiHeadAttention,
# AdditiveAttention after LSTMs, and specific dtype handling for key layers.
...
# ModelBuilder for Version 4 will be instantiated by MainClass based on parameters passed.
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run Time Series Forecasting Model (Version 4 - Standard Configuration)",
    ...
    # ModelBuilder V4 specific parameters
...
def main(cli_args):
    ...
    and runs the Version 4 model pipeline.
    """
    SCRIPT_LOGGER.info(f"--- Initializing Model Pipeline for Version 4 (Standard Run) ---")
    ...
    default_time_steps = 3 # From original V4 DataLoader defaults (or adjust as needed)
    ...
    # --- Default Model Architecture Hyperparameters for ModelBuilder Version 4 ---

    # `block_configs_v4`: Defines the structure of the convolutional residual blocks.
    # This should match the structure intended for V4. Example from original V4 Run.py:
    default_block_configs_v4 = [
        {'filters': 8, 'kernel_size': 3, 'pool_size': None},
        # Add more block configurations here if the V4 architecture uses multiple distinct residual blocks.
    ]

    # `model_builder_params_v4`: Holds other architectural hyperparameters for ModelBuilder V4.
    # Keys must match arguments in ModelBuilder V4's __init__.
    # Values align with V8 where comparable, or use V4's original/sensible defaults.
    default_model_builder_params_v4 = {
        'num_heads_residual_block': 3,      # Original V4 ModelBuilder default
        'key_dim_residual_block': 4,         # Original V4 ModelBuilder default
        'leaky_relu_alpha_conv_1': 0.04,     # Original V4 ModelBuilder default
        'leaky_relu_alpha_conv_2': 0.03,     # Original V4 ModelBuilder default
        'leaky_relu_alpha_after_residual_add': 0.03, # Original V4 ModelBuilder default
        'conv_l2_reg': 0.0,

        'num_bilstm_layers': 1,              # Original V4 ModelBuilder had one BiLSTM
        'lstm_units': 200,                   # Matches V8 and original V4
        'recurrent_dropout_lstm': 0.0,       # Always off as per user request
        'lstm_l2_reg': 0.0,
        'use_batchnorm_after_lstm': False,    # Defaulting to False

        'use_batchnorm_after_final_attention': True, # As per original V4 ModelBuilder (after AdditiveAttention)
    ...
    model_builder_params_v4 = default_model_builder_params_v4.copy()
    def update_param(param_name, cli_value, params_dict): # Helper to avoid repeating getattr
        if cli_value is not None:
            params_dict[param_name] = cli_value

    update_param('optimizer_lr', cli_args.optimizer_lr, model_builder_params_v4)
    update_param('num_heads_residual_block', cli_args.num_heads_residual_block, model_builder_params_v4)
    update_param('key_dim_residual_block', cli_args.key_dim_residual_block, model_builder_params_v4)
    update_param('leaky_relu_alpha_conv_1', cli_args.leaky_alpha_conv1, model_builder_params_v4)
    update_param('leaky_relu_alpha_conv_2', cli_args.leaky_alpha_conv2, model_builder_params_v4)
    update_param('leaky_relu_alpha_after_residual_add', getattr(cli_args, 'leaky_alpha_after_residual_add', None), model_builder_params_v4)
    update_param('conv_l2_reg', cli_args.conv_l2_reg, model_builder_params_v4)
    update_param('num_bilstm_layers', cli_args.num_bilstm_layers, model_builder_params_v4)
    update_param('lstm_units', cli_args.lstm_units, model_builder_params_v4)
    update_param('lstm_l2_reg', cli_args.lstm_l2_reg, model_builder_params_v4)
    update_param('output_l2_reg', cli_args.output_l2_reg, model_builder_params_v4)

    # block_configs_v4 is more complex for CLI, using default. Can be extended (e.g., load from JSON).
    block_configs_v4 = default_block_configs_v4

    # --- Output Directory for this Run ---
    default_output_base_dir = os.path.join(script_base_dir, "IEEE_TNNLS_Runs_V4_Standard_MP_NoDropout")
    ...
    # -----------------------------------------------------------------------
    SCRIPT_LOGGER.info(f"--- Final Configuration for Version 4 Run ---")
    ...
    SCRIPT_LOGGER.info(f"  ModelBuilder V4 - Block Configurations: {json.dumps(block_configs_v4, indent=4)}")
    SCRIPT_LOGGER.info(f"  ModelBuilder V4 - Other Architectural Parameters:")
    for key, value in model_builder_params_v4.items():
        SCRIPT_LOGGER.info(f"    {key}: {value}")

    # -----------------------------------------------------------------------
    # Initialize and Execute the Time Series Model Pipeline
    # -----------------------------------------------------------------------
    SCRIPT_LOGGER.info("Initializing TimeSeriesModel pipeline with Version 4 configuration...")
    try:
        time_series_pipeline = TimeSeriesModel(
            ...
            block_configs=block_configs_v4, # Crucial for V4 ModelBuilder
            model_builder_params=model_builder_params_v4
        )

        SCRIPT_LOGGER.info("🚀 Starting the pipeline execution for Version 4...")
        time_series_pipeline.run()
        SCRIPT_LOGGER.info("🎉 Pipeline execution for Version 4 completed successfully.")

    except Exception as e:
        SCRIPT_LOGGER.critical(
            f"❌ A critical error occurred during the pipeline execution for Version 4: {e}",
            exc_info=True
        )
        sys.exit(1)
