...
# an AdditiveAttention layer, and a Dense output layer.
# Hyperparameters are managed externally via the constructor.
...
from tensorflow.keras.layers import (
    ...
    MultiHeadAttention,
    Add,
    AdditiveAttention # As specified for V4
)
...
# It's generally recommended to set seeds at the main script level (e.g., in Run.py)
...
class ModelBuilder:
    """
    Builds a time series forecasting model with Version 4 architecture:
    Input -> [Configurable Residual Blocks with Conv1D & MHA] -> BiLSTM_Block -> AdditiveAttention -> Dense_Output.
    ...
    def __init__(self,
                 ...
                 num_heads_residual_block: int = 12, # Original V4 default
                 key_dim_residual_block: int = 4,    # Original V4 default
                 leaky_relu_alpha_conv_1: float = 0.04, # Original V4 default
                 leaky_relu_alpha_conv_2: float = 0.03, # Original V4 default
                 leaky_relu_alpha_after_residual_add: float = 0.03, # Original V4 default
                 ...
                 use_batchnorm_after_lstm: bool = True, # Defaulting to True for stability
                 # Final AdditiveAttention Layer Parameters
                 use_batchnorm_after_final_attention: bool = True, # As per V4 original structure
        ...
        """
        Initializes the ModelBuilder for Version 4.
        ...
        Args:
            ...
            use_batchnorm_after_final_attention (bool): Apply BatchNormalization after the final AdditiveAttention layer.
    ...
    def _residual_block(self,
        ...
        """
        Constructs a residual block as defined in Version 4's architecture.
        ...
        Args:
            ...
            pool_size (Optional[int]): Pooling size for MaxPooling1D. If None or 1, no pooling.
        ...
        """
        shortcut = x_input
        x = x_input # Start with x_input for the main path

        # First Conv1D -> BN -> LeakyReLU -> MHA -> BN sequence
        x = Conv1D(filters=filters, kernel_size=kernel_size, padding='same',
                   kernel_regularizer=self.conv_kernel_regularizer, name=f'resblock{block_num}_conv1')(x)
        ...
        x = mha1(query=x, value=x, key=x) # Self-attention
        ...
        if pool_size is not None and pool_size > 1:
            ...
            # If pooling changes sequence length, shortcut needs to be adjusted too.
            # This adjustment is handled before the Add layer.
        ...
        x = mha2(query=x, value=x, key=x) # Self-attention
        ...
        # Check if number of filters changed or if sequence length changed due to pooling
        if shortcut.shape[-1] != filters or shortcut.shape[-2] != x.shape[-2]:
            ...
            # This logic assumes symmetric pooling in the main path if pool_size was applied.
            if x_input.shape[-2] != x.shape[-2]: # If sequence length of original input changed
                current_shortcut_len = x_input.shape[-2]
                target_x_len = x.shape[-2]
                
                # This simplified pooling on shortcut assumes if x was pooled, shortcut needs similar reduction.
                # A more robust way might involve calculating total stride from x's pooling layers.
                if pool_size is not None and pool_size > 1:
                    # Apply pooling to shortcut to match x's dimension reduction from two pooling layers
                    if MaxPooling1D(pool_size=pool_size, padding='same')(MaxPooling1D(pool_size=pool_size, padding='same')(x_input)).shape[-2] == target_x_len:
                    ...
                    elif MaxPooling1D(pool_size=pool_size, padding='same')(x_input).shape[-2] == target_x_len:
                         shortcut = MaxPooling1D(pool_size=pool_size, padding='same', name=f'resblock{block_num}_shortcut_pool1')(shortcut)
    ...
    def build_model(self) -> Model:
        """
        Constructs and compiles the Keras model based on Version 4 architecture.
        ...
        input_layer = Input(
            ...
            dtype='float32', # Explicitly set dtype to float32 for the input
        ...
        for i, block_config_dict in enumerate(self.block_configs):
            ...
            pool_size = block_config_dict.get('pool_size', None) # Pooling applied within _residual_block
        ...
        for i in range(self.num_bilstm_layers):
            x = Bidirectional(
                LSTM(
                    ...
                    return_sequences=True, # True for feeding into AdditiveAttention
        ...
        # --- Final AdditiveAttention Layer ---
        # AdditiveAttention (Bahdanau-style) requires query and value.
        # For self-attention on the LSTM sequence:
        # Query can be the last hidden state, or the whole sequence if context is needed for each step.
        # Here, we apply self-attention over the entire sequence output from LSTM.
        # Keras AdditiveAttention usually expects query, value, key.
        # For self-attention, query=value=key=x (the LSTM output sequence).
        # However, AdditiveAttention is simpler: attention_output = layer([query, value])
        # If we want a context vector, we might need a different setup or use it as is.
        # The original V4 ModelBuilder.py used:
        # attention_layer = AdditiveAttention()
        # x = attention_layer([x, x])
        # This implies x is both query and value.
        attention_output = AdditiveAttention(name='final_additive_attention')([x, x])
        ...
        # --- Model Definition ---
        model = Model(inputs=input_layer, outputs=output, name='TimeSeriesForecastingModel_V4')
