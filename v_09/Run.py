...
def setup_environment(enable_mixed_precision: bool = True):
    """Configures the global execution environment."""
    ...
    SCRIPT_LOGGER.info(f"Global TensorFlow seed set to {SEED}.")
    K.clear_session()
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Run Time Series Forecasting Model (Version 9 - Standardized)", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def main(cli_args):
    """Main execution function for the Version 9 pipeline."""
    SCRIPT_LOGGER.info("--- Initializing Pipeline for Model Version 9 ---")
    
    script_base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # --- Default Configurations for Version 9 (Corrected) ---
    ...
    default_model_params = {
        'leaky_relu_alpha_res_block_1': 0.04, 'leaky_relu_alpha_res_block_2': 0.03, 'leaky_relu_alpha_after_add': 0.03,
        'conv_l2_reg': 0.0, 'lstm_units': 200, 'recurrent_dropout_lstm': 0.0, 'lstm_l2_reg': 0.0,
        'use_batchnorm_after_lstm': False, 'use_batchnorm_after_attention': True,
        'moe_num_experts': 12, 'moe_units': 64, 'moe_leaky_relu_alpha': 0.01,
        'use_batchnorm_after_moe': True, 'output_l2_reg': 0.0,
        'optimizer_lr': 0.01, 'optimizer_clipnorm': False
    }
    ...
    output_dir = cli_args.output_dir or os.path.join(script_base_dir, "IEEE_TNNLS_Runs_V9")
    os.makedirs(output_dir, exist_ok=True)

    # --- Comprehensive Final Configuration Logging ---
    SCRIPT_LOGGER.info("--- Final Configuration for Version 9 Run ---")
    ...
    SCRIPT_LOGGER.info(f"  ModelBuilder V9 - Block Configurations: {json.dumps(default_block_configs, indent=4)}")
    SCRIPT_LOGGER.info(f"  ModelBuilder V9 - Other Architectural Hyperparameters:")
    ...
    try:
        ...
        pipeline.run()
        SCRIPT_LOGGER.info("🎉 Version 9 pipeline completed successfully.")
    except Exception as e:
        SCRIPT_LOGGER.critical(f"❌ Version 9 pipeline failed: {e}", exc_info=True)
        sys.exit(1)
