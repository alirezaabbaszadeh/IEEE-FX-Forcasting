...
def setup_environment(enable_mixed_precision: bool = True):
    """
    Configures the global execution environment, including logging, GPU memory, and Keras session.
    """
    logging.basicConfig(
        level=logging.INFO,
    ...
    if gpus:
        try:
            ...
            SCRIPT_LOGGER.error(f"GPU setup failed: {e}", exc_info=True)
    else:
        SCRIPT_LOGGER.info("No GPU detected.")

def parse_arguments():
    """
    Parses command-line arguments to configure the pipeline run flexibly.
    """
    parser = argparse.ArgumentParser(
        description="Run Time Series Forecasting Model (Version 8 - Fully Standardized)",
...
def main(cli_args):
    """
    Main execution function for the Version 8 pipeline.
    """
    SCRIPT_LOGGER.info("--- Initializing Pipeline for Model Version 8 ---")
    
    script_base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # --- Default Configurations for Version 8 ---
    ...
    default_model_params = {
        'num_heads_res_block': 3, 'key_dim_res_block': 4,
        'lstm_units': 200, 'num_lstm_layers': 1,
        'attention_after_lstm_heads': 6, 'attention_after_lstm_key_dim': 10,
        'moe_num_experts': 12, 'moe_units': 64,
        'dense_units_before_output': 256,
        'leaky_relu_alpha_dense': 0.00, 'leaky_relu_alpha_res_block': 0.04,
        'leaky_relu_alpha_res_block2': 0.03, 'dropout_rate': 0.0, 'l2_reg': 0.0,
        'optimizer_lr': 0.01
    }
    ...
    output_dir = cli_args.output_dir or os.path.join(script_base_dir, "IEEE_TNNLS_Runs_V8")
    os.makedirs(output_dir, exist_ok=True)

    # --- Comprehensive Final Configuration Logging ---
    SCRIPT_LOGGER.info("--- Final Configuration for Version 8 Run ---")
    ...
    SCRIPT_LOGGER.info(f"  ModelBuilder V8 - Block Configurations: {json.dumps(default_block_configs, indent=4)}")
    SCRIPT_LOGGER.info(f"  ModelBuilder V8 - Other Architectural Hyperparameters:")
    ...
    try:
        ...
        pipeline.run()
        SCRIPT_LOGGER.info("🎉 Version 8 pipeline completed successfully.")
    except Exception as e:
        SCRIPT_LOGGER.critical(f"❌ Version 8 pipeline failed: {e}", exc_info=True)
        sys.exit(1)
