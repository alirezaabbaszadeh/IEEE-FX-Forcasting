...
### Optional appendix: purged stacking ensemble
The stacking pipeline now writes two artefacts alongside the primary metrics export:
`stacking_weights.csv` and `stacking_fold_metrics.csv`. The former records the embargo-aware
weights allocated to each base model, while the latter tracks fold-level MSE diagnostics used to
derive those weights. When preparing appendix material, join the per-run weight files into
`paper_outputs/stacking/stacking_appendix_table.csv` so reviewers can trace ensemble gains back to
individual models and validation folds. The optional appendix table mirrors the schema of the
weights file and is generated by calling `scripts/export_tables.py` on the collected stacking
artefacts.
...
- **Appendix Table A1 (optional):** Purged stacking weights aggregated across validation folds.
